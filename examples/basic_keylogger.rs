...
use interception_copilot::{
    Context, InterceptionFilter, KeyStroke, Stroke,
    is_keyboard_device, keyboard,
};
                    ...
                    Ok(strokes) => {
                        for stroke in &strokes {
                            unsafe {
                                ...
                                let key_action = if key_stroke.state & 0x01 != 0 { "UP" } else { "DOWN" };
                                println!("Key {}: {} (state: 0x{:02X}, info: 0x{:08X})", 
                                    key_action, key_stroke.code, key_stroke.state, key_stroke.information);
                            }
                        }
